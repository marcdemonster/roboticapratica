...
using namespace std;
    ...
    ArvoreAVL::ArvoreAVL()//mudou o nome
    {
        raiz = NULL;
    }
    ArvoreAVL::~ArvoreAVL()//mudou o nome
    ...
    void ArvoreAVL::removerbusca(Aluno aluno, No*& noatual, bool& diminuiu) // modicicada
    {//modificada
        if(aluno.obterRa() < noatual->aluno.obterRa()){
    ...
    void ArvoreAVL::deletarNo(No*& noatual, bool& diminuiu)//modificada
    {
        No* temp = noatual;
  ...
  // novos métodos:
    ...
    void ArvoreAVL::realizarotacao(No*& pai)
    {
        ...
        if (filho ->fatorB == -1){
            ...
            filho->fatorB= 1;
            rotacaodireita(pai);
        }else if(filho->fatorB == 1){// caso precise da rotacao dupla
